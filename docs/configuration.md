...
HTTP (`:54545`) is used for firmware update and network diagnostic sensors when that management interface is reachable. Devices without a listener (for example some gRPC-only models) skip those entities. Changing transport requires removing and re-adding the integration (entity set differs per mode).
