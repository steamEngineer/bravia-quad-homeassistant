...
**External control TCP probe:** After `GetCapabilities`, gRPC setup/reconnect skips the `:33336` external-control ensure when the device does not advertise `system_setting.external_control` (gRPC-only models such as HT-A8). If capabilities fetch soft-fails, the legacy TCP → gRPC probe still runs.
