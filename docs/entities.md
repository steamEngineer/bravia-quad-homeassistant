...
| `sensor.bravia_quad_*_mac_wired` | Sensor | MAC address (wired) | — | gRPC only when GetCapabilities advertises `system_setting.wifi_mac_address_wired`; otherwise HTTP when the management FCGI on `:54545` responds and the device exposes it. Not created when capabilities omit the path or GetCapabilities was unavailable |
...
| `update.bravia_quad_*_firmware_update` | Update | Firmware update | — | HTTP when the management FCGI on `:54545` responds |
