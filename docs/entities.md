...
- **Media player** — power, volume, mute, source, sound field mode (`sound_setting.sound_effect`), now-playing metadata, and play/pause/next on streaming inputs
- **gRPC-only controls** — DSEE Ultimate, 360SSM height, center speaker, DTS Dialog Control, subwoofer level (with sub), dual mono, and more
