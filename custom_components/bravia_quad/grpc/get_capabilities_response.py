...
def filter_field_paths(
    ...
    Only paths advertised in GetCapabilities are kept. Devices reject the entire
    bulk GetStates when any requested path is invalid, so metadata siblings
    (``*.availability`` / ``*.unavailable_reason`` / ``*.range``) must be
    advertised themselves — inferring them from a base path is unsafe.
